            ...
            void FillFrom(const std::vector<T> &source) override {
                if (source.size() != this->rows_ * this->cols_) {
                    ...
                    for (int i = 0; i < this->rows_; i++) {
                        for (int j = 0; j < this->cols_; j++)data_[i][j] = source[cnt++];
                    }
                }
            }
            // NotImplementedException{ "RowMatrix::FillFrom() not implemented." };
    ...
    public:
        ...
        static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T>* matrixA, const RowMatrix<T>* matrixB) {
            ...
            if ((matrixA->GetRowCount() != matrixB->GetRowCount()) || (matrixA->GetColumnCount() != matrixB->GetColumnCount())) {
                ...
                auto rst = std::make_unique<RowMatrix<T>>(r, c);
                for (int i = 0; i < r; i++) {
                    for (int j = 0; j < c; j++) {
                        rst->SetElement(i, j, matrixA->GetElement(i, j) + matrixB->GetElement(i, j));
                    }
                }
                return rst;
            }
        ...
        static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T>* matrixA, const RowMatrix<T>* matrixB) {
            ...
            if (matrixA->GetColumnCount() != matrixB->GetRowCount()) {
                ...
                int r = matrixA->GetRowCount();
                int c = matrixB->GetColumnCount();
                auto rst = std::make_unique<RowMatrix<T>>(r, c);
                for (int i = 0; i < r; i++) {
                    for (int j = 0; j < c; j++) {
                        int mid = 0;
                        for (int k = 0; k < matrixA->GetColumnCount(); k++) {
                            mid += matrixA->GetElement(i, k) * matrixB->GetElement(k, i);
                        }
                        rst->SetElement(i, j, mid);
                    }
                }
                return rst;
            }
